...
// use crate::chain::network::{InvType, Message, Request};
// use crate::timeout::{ExpectedReply, SharedTimeout};
// use crate::downstream::SharedDownstream;
// use crate::manager::peer_manager::Error;
// use crate::network::p2p::layer::{P2PControl, P2PControlSender, PeerId, PeerMessage, PeerMessageReceiver, PeerMessageSender, PeerMessageSenderWrapper};
// use crate::util::Shared;
//
// pub struct HeadersFlow {
...
//
// impl HeadersFlow {
//     pub fn new(chain: Shared<Chain>, p2p: P2PControlSender<NetworkMessage>, timeout: SharedTimeout<NetworkMessage, ExpectedReply>, downstream: SharedDownstream) -> PeerMessageSenderWrapper<Message> {
//         let (sender, receiver) = mpsc::sync_channel(p2p.back_pressure);
//         let mut flow = HeadersFlow { chain, p2p, timeout, downstream };
//         thread::Builder::new().name("headers flow".to_string()).spawn(move || flow.run(receiver)).unwrap();
//         PeerMessageSenderWrapper::new(sender)
//     }
//
...
//                 self.chain.read(|chain| chain.last_terminal_block)
//                 let chaindb = self.chain.read().unwrap();
...
//             self.p2p.send_network(peer, Request::GetHeaders())
...
//                 {
//                     let mut chaindb = self.chain.write().unwrap();
