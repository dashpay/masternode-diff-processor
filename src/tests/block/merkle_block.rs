use byte::BytesExt;
use hashes::hex::FromHex;
use crate::chain::block::{IBlock, merkle_block, MerkleBlock};
use crate::chain::common::ChainType;
use crate::chains_manager::ChainsManager;
use crate::crypto::{byte_util::Reversable, UInt256};
use crate::util::Shared;

#[test]
fn test_merkle_block() {
    /*
     // -> block 745465 in all it's glory <-
     {
         "hash": "0000000000000197df9123a822ae2ff2b1108b37d641b2a8c976ba949a78ca51",
         "confirmations": 4383,
         "size": 953,
         "height": 745465,
         "version": 536870912,
         "merkleroot": "d5d706130205b588d88977de7438399d8106a7446c4b300c7f902a0f3c16ad2a",
         "tx": [
         "e975d526d29ef6967249539493a34aa63d14f701dfbab83ba7188e93599f9e27",
         "f68abb43a46d00b993f6c0f2df2a9dc377bb0ffbe0583f65d5186b92be403255",
         "2f5709c4d50c2574e8717d1efd9bc9a0a90da3f04dff46660cb54dba2ecb2555",
         "c24891989f089f671967d363df3c184520f4961f16037600478b18503d2c319b"
         ],
         "time": 1506710174,
         "mediantime": 1506709673,
         "nonce": 125766022,
         "bits": "1a023d68",
         "difficulty": 7490155.121260014,
         "chainwork": "000000000000000000000000000000000000000000000010796b646d372a9997",
         "previousblockhash": "00000000000000a4570dff221ace766642e7f7cd59bbedf12dd2c3ff884e5d1d",
         "nextblockhash": "0000000000000021850b1e5d87bd92aa0e52a6f0de2e9363cd7336f22a479b02"
     }
     */
    // this block is bloom filtered to only have the first transaction
    let manager = ChainsManager::new();
    let chain_type = ChainType::TestNet;
    let read_context = merkle_block::ReadContext(chain_type, Shared::None);
    let block = Vec::from_hex("").unwrap();
    let b = block.as_slice().read_with::<MerkleBlock>(&mut 0, read_context).unwrap();
    let hash = UInt256::from_hex("0000000000000197df9123a822ae2ff2b1108b37d641b2a8c976ba949a78ca51").unwrap().reversed();
    assert_eq!(b.block_hash(), hash, "block hash is wrong");
    assert_eq!(block, b.to_data(), "block data is wrong");
    assert!(b.is_valid(), "block is not valid");
    let hash = UInt256::from_hex("e975d526d29ef6967249539493a34aa63d14f701dfbab83ba7188e93599f9e27").unwrap().reversed();
    assert!(b.contains_tx_hash(hash), "block doesn't contain tx hash");
    assert_eq!(b.base.total_transactions, 4, "block has wrong number of transactions");
    let hash = UInt256::from_hex("e975d526d29ef6967249539493a34aa63d14f701dfbab83ba7188e93599f9e27").unwrap().reversed();
    assert_eq!(b.transaction_hashes().get(0).unwrap(), &hash, "block has wrong first transaction hash");
    // TODO: test a block with an odd number of tree rows both at the tx level and merkle node level
    // TODO:XXXX test verifyDifficultyFromPreviousBlock
}
