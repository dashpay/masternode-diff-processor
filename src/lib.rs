...
mod tests {
    ...
    use crate::{BlockHeightLookup, ffi, llmq_rotation_info_process2, MasternodeList, MerkleTree, mndiff_process, mnl_diff_process, ToFFI};
    ...
    fn assert_diff_result(chain: ChainType, result: ffi::types::MNListDiffResult) {
    ...
    fn load_masternode_lists_for_files<'a, BHL: Fn(UInt256) -> u32 + Copy>(files: Vec<String>, chain: ChainType, block_height_lookup: BHL) -> (bool, HashMap<UInt256, MasternodeList<'a>>) {
    ...
    fn test_mainnet_reload() {
        ...
        let (success, lists) = load_masternode_lists_for_files(
            files,
            chain,
            |block_hash| block_height_for(chain, block_hash.clone().reversed().0.to_hex().as_str())
        );
