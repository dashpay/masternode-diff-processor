...
pub mod tests {
    ...
    pub unsafe extern "C" fn block_height_lookup_default(block_hash: *mut [u8; 32], context: *const std::ffi::c_void) -> u32 {
        let block_hash = UInt256(*block_hash);
        let data: &mut FFIContext = &mut *(context as *mut FFIContext);
        block_height_for(data.chain, block_hash.clone().reversed().0.to_hex().as_str())
    }
